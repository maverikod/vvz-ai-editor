...
VERSION defaults to the root VERSION file (the one number shared by the server,
the client and the engine). Image tags: REPO:VERSION and REPO:latest.
...
if [ -z "$TAG" ]; then
  ...
  TAG="$(tr -d '[:space:]' < "$PROJECT_ROOT/VERSION")"
fi
